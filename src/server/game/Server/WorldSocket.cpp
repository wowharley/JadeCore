    ...
    try
    {
        switch (opcode)
        {
            ...
            default:
            {
                ...
                {
					TC_LOG_DEBUG("network.opcode", "No defined handler for opcode %s sent by %s", GetOpcodeNameForLogging(new_pct->GetOpcode(), false, new_pct->GetReceivedOpcode()).c_str(), m_Session->GetPlayerInfo().c_str());
                    return 0;
                }
