...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
				...
				{
					player->GetSession()->SendNotification("You are in Feral Form! Shapeshift back to normal to be able to morph!");
                    player->CLOSE_GOSSIP_MENU();
                    return false;
		        }
                ...
                {
					player->GetSession()->SendNotification("You are shapeshifted! Shapeshift back to normal to be able to morph!");
                    player->CLOSE_GOSSIP_MENU();
                    return false;
		        }
								...
								player->ADD_GOSSIP_ITEM(6, "Draenei Female", GOSSIP_SENDER_MAIN, 22);
								player->ADD_GOSSIP_ITEM(6, "Draenei Male", GOSSIP_SENDER_MAIN, 23);
								player->ADD_GOSSIP_ITEM(6, "Worgen Female", GOSSIP_SENDER_MAIN, 24);
								player->ADD_GOSSIP_ITEM(6, "Worgen Male", GOSSIP_SENDER_MAIN, 25);
								player->ADD_GOSSIP_ITEM(6, "Fire Kitty", GOSSIP_SENDER_MAIN, 26);
								player->ADD_GOSSIP_ITEM(6, "Demorph My Character", GOSSIP_SENDER_MAIN, 27);
            ...
            bool OnGossipSelect(Player* player, Creature* creature, uint32 uiSender, uint32 action)
            {
				if (player->IsMounted())
					...
					if (player->IsInFeralForm())
					{
						player->GetSession()->SendNotification("You are in Feral Form! Shapeshift back to normal form to be able to morph!");
                        player->CLOSE_GOSSIP_MENU();
                        return false;
		            }
					...
					if (player->HasAura(40120) || player->HasAura(33943) || player->HasAura(783) || player->HasAura(5487) || player->HasAura(24858) || player->HasAura(33891))
                   {
						player->GetSession()->SendNotification("You are shapeshifted! Shapeshift back to normal form to be able to morph!");
                        player->CLOSE_GOSSIP_MENU();
                        return false;
		            }
